      ...
      {pairing.code && pairing.countdown > 0 ? (
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            <Button variant="outline" size="sm" className="cursor-pointer" onClick={pairing.copyCommand}>
              {pairing.copied ? <Check className="w-3.5 h-3.5 text-success" /> : <Copy className="w-3.5 h-3.5" />}
          ...
          <p className="text-xs text-muted-foreground/80">
            在 {channelName} 中向 Bot 发送 <code className="bg-surface-2 px-1 rounded">/pair {pairing.code}</code> 完成配对
          </p>
        </div>
      ) : (
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground/80">
            ...
            <code className="bg-surface-2 px-1 rounded">/pair &lt;code&gt;</code> 将聊天绑定到此账号
          </p>
        </div>
      )}
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            className="text-xs text-muted-foreground/80 hover:text-muted-foreground disabled:opacity-50"
          >
            刷新
          </button>
        </div>
        {paired.loading ? (
          <div className="text-xs text-muted-foreground/80">加载中...</div>
        ) : paired.chats.length === 0 ? (
          <div className="text-xs text-muted-foreground/80">暂无已配对的聊天</div>
        ) : (
          <div className="space-y-1.5">
            {paired.chats.map((chat) => (
              <div key={chat.jid} className="flex items-center justify-between px-3 py-2 rounded-lg bg-surface group">
                <div className="min-w-0">
                  <div className="text-sm text-foreground truncate">{chat.name}</div>
                  <div className="text-xs text-muted-foreground/80">{new Date(chat.addedAt).toLocaleString('zh-CN')}</div>
                </div>
                <button
                  onClick={() => paired.remove(chat.jid)}
                  disabled={paired.removingJid === chat.jid}
                  className="ml-2 p-1 rounded text-muted-foreground/80 hover:text-error hover:bg-error-bg opacity-0 group-hover:opacity-100 transition-all disabled:opacity-50"
                  title="移除配对"
                >

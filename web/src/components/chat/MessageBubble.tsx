        ...
        <div className="mt-1.5 space-y-0 max-h-48 overflow-y-auto">
          {blocks.map((block) => {
            ...
            if (block.type === 'status') {
              return (
                <div key={block.id} className="flex items-center gap-1.5 py-0.5 text-[11px] text-warning">
                  <span>&#9888;</span>
...
function ReasoningBlock({ content }: { content: string }) {
  ...
  return (
    <div className="mb-3 rounded-xl border border-warning-border/60 bg-warning-bg/40 overflow-hidden">
      <button
        onClick={() => setExpanded(!expanded)}
        className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-warning-bg/60 transition-colors"
      >
        ...
        <span className="text-xs font-medium text-warning">Reasoning</span>
        <span className="flex-1" />
        {expanded ? (
          <ChevronUp className="w-3.5 h-3.5 text-warning" />
        ) : (
          <ChevronDown className="w-3.5 h-3.5 text-warning" />
        )}
      </button>
      {expanded && (
        <div className="px-3 pb-3 text-sm text-warning-foreground/70 whitespace-pre-wrap break-words max-h-64 overflow-y-auto border-t border-warning-border">
          {content}
        </div>
      )}
    </div>
      ...
      <div className="mb-6">
        {showTime && (
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-error">系统消息</span>
          </div>
        )}
        <div className="relative bg-error-bg rounded-xl border border-error-border border-l-[3px] border-l-error px-5 py-4">
          <div className="flex items-start gap-3">
            <div className="flex-shrink-0 w-6 h-6 bg-error rounded-full flex items-center justify-center text-background font-bold text-sm">
              !
            </div>
            <div className="flex-1">
              <h3 className="text-sm font-semibold text-error-foreground mb-1">上下文溢出错误</h3>
              <p className="text-sm text-error-foreground leading-relaxed">{errorMsg}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  if (message.sender === '__task__') {
    ...
    return (
      <div className="mb-6">
        ...
        <div className="flex items-start gap-3">
          <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-warning-bg flex items-center justify-center text-warning text-sm">
            ⏰
          </div>
          <div className="bg-warning-bg border border-warning-border rounded-xl px-4 py-3 max-w-[80%]">
            <div className="text-xs text-warning font-medium mb-1">定时任务</div>
    ...
    return (
      <div className="group mb-2 border-b border-border pb-2" onContextMenu={handleContextMenu} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove}>
        ...
        <div className="flex items-center gap-1.5 mb-1">
          ...
          {showTime && <span className="text-[11px] text-muted-foreground/80">{time}</span>}
          <button
            onClick={handleCopy}
            className="ml-1 w-5 h-5 rounded flex items-center justify-center text-muted-foreground/50 hover:text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
            title="复制"
          >
      ...
      return (
        <div className="group mb-4" onContextMenu={handleContextMenu} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove}>
          <div className="flex items-center gap-2 mb-1.5 lg:hidden">
            <div className="w-6 h-6 rounded-full bg-surface-3 flex items-center justify-center text-xs font-medium text-muted-foreground flex-shrink-0">
              {initial}
            </div>
          ...
          <div className="lg:flex lg:gap-3">
            <div className="hidden lg:block flex-shrink-0">
              <div className="w-8 h-8 rounded-full bg-surface-3 flex items-center justify-center text-sm font-medium text-muted-foreground">
                {initial}
              </div>
            </div>
    ...
    return (
      <div className="group flex justify-end mb-4" onContextMenu={handleContextMenu} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} onTouchMove={handleTouchMove}>
        <div className="flex flex-col items-end min-w-0 w-full">
          ...
          {showTime && (
            <span className="text-xs text-muted-foreground/80 mt-1.5 mr-1">{time}</span>
          )}
        </div>

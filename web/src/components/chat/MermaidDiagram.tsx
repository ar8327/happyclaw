    ...
    return (
      <div className="my-4 rounded-lg bg-muted border border-border p-8 flex items-center justify-center">
        <div className="animate-pulse flex flex-col items-center gap-2">
          ...
          <span className="text-sm text-muted-foreground/80">Mermaid 图表渲染中...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="relative group my-4 overflow-hidden">
        <div className="absolute right-2 top-2 opacity-70 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
          <button
            onClick={handleCopy}
            className="p-2 rounded-lg bg-surface-3 hover:bg-border-strong text-foreground text-xs flex items-center gap-1"
          >
            {copied ? (
              <>
                <Check size={14} />
        ...
        <div className="text-xs text-warning bg-warning-bg border border-warning-border rounded-t-lg px-3 py-1">
      ...
      {expanded && createPortal(
        ...
        >
          ...
          >
            <button
              onClick={() => setExpanded(false)}
              className="absolute top-3 right-3 z-20 p-2 rounded-full bg-foreground/70 text-background hover:bg-foreground transition-colors cursor-pointer"
              aria-label="关闭图表预览"
              title="关闭"
            >
              <X size={16} />

...
function MarkdownImage({ src, alt }: { src?: string; alt?: string }) {
  ...
  if (error) {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-1 bg-surface-2 text-muted-foreground rounded text-sm">
    ...
    <div className={textSizeClass}>
      <ReactMarkdown
        ...
        components={{
          ...
          blockquote: ({ children }) => (
            <blockquote className="border-l-4 border-border-strong pl-4 my-4 text-muted-foreground italic">
              {children}
            </blockquote>
          ),
        }}

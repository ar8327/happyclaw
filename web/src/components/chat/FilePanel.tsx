...
function FileIcon({ name }: { name: string }) {
  ...
  if (IMAGE_EXTENSIONS.has(ext))
    return <Image className="w-4 h-4 text-[var(--tag-rose-fg)]" />;
  ...
  return <File className="w-4 h-4 text-muted-foreground/80" />;
}
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border flex-shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            {dirty && (
              <span className="text-xs text-warning flex-shrink-0">未保存</span>
            )}
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button
              onClick={onClose}
              className="text-muted-foreground/80 hover:text-muted-foreground transition-colors p-2 rounded-md hover:bg-muted cursor-pointer"
              aria-label="关闭编辑器"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        {/* Footer hint */}
        <div className="px-4 py-2 border-t border-border-subtle text-xs text-muted-foreground/80 flex-shrink-0">
...
export function FilePanel({ sessionId, onClose }: FilePanelProps) {
  ...
  return (
    <div className="w-full h-full border-l border-border bg-background flex flex-col">
        ...
        <div className="flex items-center gap-1">
          {canOpenLocalFolder && (
            <button
              ...
              className="hidden md:inline-flex text-muted-foreground/80 hover:text-muted-foreground transition-colors p-2 rounded-md hover:bg-muted cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              title="打开工作区文件夹"
              aria-label="打开工作区文件夹"
            >
      ...
      {openDirError && (
        <div className="px-4 py-2 border-b border-error-border bg-error-bg text-xs text-error">
          {openDirError}
        </div>
      )}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`text-sm truncate ${
                          item.isSystem ? 'text-muted-foreground/80' : 'text-foreground'
                        }`}
                      >
                        {item.name}
                      </span>
                    ...
                    {item.type === 'file' && (
                      <p className="text-[11px] text-muted-foreground/80 leading-tight">{formatSize(item.size)}</p>
                    )}
                  </div>
                    ...
                    <div className="flex-shrink-0 flex items-center gap-0.5">
                      ...
                      {item.type === 'file' && TEXT_EXTENSIONS.has(getFileExt(item.name)) && (
                        <button
                          ...
                          className="p-2.5 rounded hover:bg-brand-100 text-muted-foreground/80 hover:text-primary transition-colors cursor-pointer"
                          title="编辑"
                          aria-label="编辑文件"
                        >
                      ...
                      {item.type === 'file' && (
                        <button
                          ...
                          className="p-2.5 rounded hover:bg-surface-3 text-muted-foreground/80 hover:text-muted-foreground transition-colors cursor-pointer"
                          title="下载"
                          aria-label="下载文件"
                        >
                      ...
                      <button
                        ...
                        className="p-2.5 rounded hover:bg-error-bg text-muted-foreground/80 hover:text-error transition-colors cursor-pointer"
                        title="删除"
                        aria-label="删除文件"
                      >

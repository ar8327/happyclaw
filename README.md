...
- **统一本地 runtime** — 新 Session 固定为 `runtime_mode=local`，旧 `execution_mode` 只作为兼容字段回写
...
当前所有 Session 都通过本地 runtime 启动。界面中的 `runtime_mode` 固定为 `local`，旧 `execution_mode` 仅用于兼容历史数据。

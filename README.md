...
主 Agent 基于 Claude Code CLI `claude -p` 运行，Claude provider 通过长生命周期 subprocess 会话承接流式事件、hooks 和 MCP 工具。
...
- **Memory Agent** — 每个用户一个子进程，当前仍使用 Claude Agent SDK 持久 query 会话，自动归档对话、生成印象、维护知识索引
...
> 容器模式会在镜像中安装 Claude Code CLI。宿主机模式依赖本机 `claude` 命令可用，并在启动时做能力探测。
...
| 模式 | 说明 | 适用对象 | 前置要求 |
|------|------|---------|---------|
| **宿主机模式** | Agent 直接在宿主机运行，访问本地文件系统 | admin 主工作区（`folder=main`） | 本机 `claude` 命令可用 |
...
flowchart TD
    ...
    subgraph Agent["Agent 运行时"]
        Runner["Claude Runner<br/>(claude -p 会话)"]
...
**数据流**：消息从接入层进入主进程，经去重和路由后分发到并发队列。队列启动宿主机进程或 Docker 容器，内部的 agent-runner 通过 Claude Code CLI `claude -p` 运行主 Agent。流式事件通过 stdout 标记协议传回主进程，经 WebSocket 广播到 Web 客户端。IM 消息由 Agent 通过 `send_message` MCP 工具显式发送，经 IPC 文件通道路由到对应 IM 渠道。Memory Agent 作为独立子进程运行，Agent 通过 `memory_query`/`memory_remember` MCP 工具经 HTTP 与之通信。
...
| **Agent** | Claude Code CLI · Claude Agent SDK · MCP SDK · IPC 文件通道 |

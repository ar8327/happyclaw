.PHONY: dev dev-backend dev-web build build-backend build-web start \
       typecheck typecheck-backend typecheck-web typecheck-agent-runner-core typecheck-agent-runner test-runner-contracts test-im-slash-command test-model-command test-steering-feishu test-feishu-conversation-mode test-im-outbox test-turn-delivery test-durable-outbound test-prompt-characterization test-memory-timeout test-memory-lanes test-memory-write-queue test-external-knowledge test-memory-search test-memory-retention test-session-list-query test-workflow-traex test-cli-model-catalog test-context-conformance \
...
# ─── Development ─────────────────────────────────────────────
	...
	@if [ ! -d node_modules ] || [ package.json -nt node_modules ] || [ ! -d web/node_modules ] || [ web/package.json -nt web/node_modules ] || [ ! -d container/agent-runner-core/node_modules ] || [ container/agent-runner-core/package.json -nt container/agent-runner-core/node_modules ] || [ ! -d container/agent-runner/node_modules ] || [ container/agent-runner/package.json -nt container/agent-runner/node_modules ]; then echo "📦 依赖有更新，安装依赖..."; $(MAKE) install; fi

dev: ensure-deps ## 启动前后端
...
start: ensure-deps ## 一键启动生产环境
	$(MAKE) build
	npm run start

# ─── Quality ─────────────────────────────────────────────────

typecheck: sync-types typecheck-backend typecheck-web typecheck-agent-runner-core typecheck-agent-runner test-runner-contracts test-im-slash-command test-model-command test-steering-feishu test-feishu-conversation-mode test-im-outbox test-turn-delivery test-durable-outbound test-prompt-characterization test-memory-timeout test-memory-lanes test-memory-write-queue test-external-knowledge test-memory-search test-memory-retention test-session-list-query test-workflow-traex test-cli-model-catalog test-context-conformance ## 全量类型检查
...
test-im-slash-command:
	npm run test:im-slash-command

test-model-command:
	npm run test:model-command
...
test-turn-delivery:
	npm run test:turn-delivery

test-durable-outbound:
	npm run test:durable-outbound

test-prompt-characterization:
	npm run test:prompt-characterization

test-memory-timeout:
	npm run test:memory-timeout

test-memory-lanes:
	npm run test:memory-lanes

test-memory-write-queue:
	npm run test:memory-write-queue

test-external-knowledge:
	npm run test:external-knowledge

test-memory-search:
	npm run test:memory-search

test-memory-retention:
	npm run test:memory-retention

test-session-list-query:
	npm run test:session-list-query

test-cli-model-catalog:
	npm run test:cli-model-catalog

test-workflow-traex:
	npm run test:workflow-traex

test-context-conformance:
	npm run test:context-conformance
...
install: ## 安装全部依赖并编译 agent-runner
	...
	npm --prefix container/agent-runner run build
	cd web && npm install
	@touch node_modules web/node_modules container/agent-runner-core/node_modules container/agent-runner/node_modules

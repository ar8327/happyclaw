...
**消息入站**：
```
飞书/Telegram/QQ/钉钉/Web 消息 → storeMessageDirect(db) + broadcastNewMessage(ws)
     ...
     → 流式输出 → onOutput 回调
         → imManager.sendFeishuMessage()/sendTelegramMessage()/sendQQMessage()/sendDingTalkMessage() + broadcastToWebClients() + db.storeMessageDirect()
```
